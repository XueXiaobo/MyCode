...
//	private Rect mTmpRect = new Rect();
	...
	// R.styleable.SwipeBackLayout, defStyle,
	// R.style.SwipeBackLayout);
	//
	// int edgeSize =
	// a.getDimensionPixelSize(R.styleable.SwipeBackLayout_edge_size, -1);
	// if (edgeSize > 0)
	// setEdgeSize(edgeSize);
	// int mode = EDGE_FLAGS[a.getInt(R.styleable.SwipeBackLayout_edge_flag,
	// 0)];
	...
	private class ViewDragCallback extends ViewDragHelper.Callback {
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			if ((mTrackingEdge & EDGE_LEFT) != 0) {
				...
				left = xvel < 0 || xvel == 0
						&& mScrollPercent > mScrollThreshold ? -(childWidth + OVERSCROLL_DISTANCE)
						: 0;
			} else if ((mTrackingEdge & EDGE_BOTTOM) != 0) {
				top = yvel < 0 || yvel == 0
						&& mScrollPercent > mScrollThreshold ? -(childHeight + OVERSCROLL_DISTANCE)
						: 0;
			}
